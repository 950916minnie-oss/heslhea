...
if not all_ships_data.empty:
    ...
    with col1: st.metric(label="⏱️ 船舶在港停留時間中位數", value=txt1)
    with col2: st.metric(label="⏳ 停靠船舶平均船齡", value=txt2)
    with col3: st.metric(label="⚖️ 平均船舶總噸位 (GT)", value=txt3)
else:
...
if not vessel_comparison_df.empty:
    ...
    if not chart1_df.empty:
        ...
        st.info("💡 該觀測條件下無個別船型的詳細在港停留時間數據。")
else:
    st.info("💡 提示：『World』或目前地區僅包含綜合 (All ships) 統計，無細分船型圖表。請從左側切換至特定國家（如 United States of America）即可觀看細分船型長條圖。")

st.markdown("---")

# 6. 船舶載重能力與噸位分析 (氣泡圖)
st.markdown("### 🚢 船舶載運規模與載重噸位 (DWT) 關
