...
st.markdown("""
    <style>
    .super-title { font-size: 46px !important; font-weight: bold !important; color: #1E3A8A !important; line-height: 1.3 !important; display: block; margin-bottom: 15px; }
    .super-sub { font-size: 30px !important; font-weight: bold !important; color: #4B5563 !important; line-height: 1.4 !important; display: block; margin-bottom: 20px; }
    [data-testid="stMetricValue"] { font-size: 30px !important; font-weight: bold !important; }
    [data-testid="stMetricLabel"] p { font-size: 18px !important; }
...
def load_data():
    ...
    if 'Unnamed: 0' in df_clean.columns: df_clean = df_clean.drop(columns=['Unnamed: 0'])
    return df_clean

df = load_data()

# 3. 側邊欄篩選 interactive selectors
...
cmap = {
    "Liquid bulk carriers": "Liquid bulk carriers<br>(液體散裝/油輪)", "Liquefied petroleum gas carriers": "LPG carriers<br>(液化石油氣船)",
    "Liquefied natural gas carriers": "LNG carriers<br>(液化天然氣船)", "Dry bulk carriers": "Dry bulk carriers<br>(乾散裝船/穀物)",
    "Dry breakbulk carriers": "Dry breakbulk<br>(雜貨船/散裝箱)", "Container ships": "Container ships<br>(貨櫃船/標準箱)"
}
if not vessel_comparison_df.empty:
    ...
    if not chart1_df.empty:
        ...
        fig_time = px.bar(chart1_df, x='Type_CN', y='Median_time_in_port_days_Value', labels={'Type_CN': '船舶類型', 'Median_time_in_port_days_Value': '在港停留天數中位數 (Days)'}, color='Median_time_in_port_days_Value', color_continuous_scale='Reds')
        fig_time.update_xaxes(tickangle=0)
        st.plotly_chart(fig_time, use_container_width=True)
    else: st.info("💡 該觀測條件下無個別船型的詳細數據。")
else: st.info("💡 提示：『World』僅包含綜合統計，切換至特定國家（如 United States of America）即可看細分船型圖。")

st.markdown("---")

# 6. 船舶載重能力與噸位分析 (氣泡圖完整復活與防禦)
st.markdown("### 🚢 船舶載運規模與載重噸位 (DWT) 關聯分析")
st.markdown("*(數據科學解讀：圓點越大代表船隻物理體積越大。)*")
if not vessel_comparison_df.empty:
    ...
    if not scatter_df.empty:
        scatter_df['Type_Scatter_CN'] = scatter_df['CommercialMarket_Label'].map(cmap).fillna(scatter_df['CommercialMarket_Label'])
        fig_scatter = px.scatter(scatter_df, x='Average_size_GT_of_vessels_Value', y='Average_cargo_carrying_capacity_dwt_per_vessel_Value', size='Average_size_GT_of_vessels_Value', color='Type_Scatter_CN', labels={'Average_size_GT_of_vessels_Value': '平均船舶總噸位 (GT)', 'Average_cargo_carrying_capacity_dwt_per_vessel_Value': '平均載重噸位 (DWT)', 'Type_Scatter_CN': '船舶類型'})
        st.plotly_chart(fig_scatter, use_container_width=True)
    else: st.info("💡 該觀測條件下缺少船隻噸位與載重能力的對應數據。")
else: st.info("💡 提示：『World』無細分船型交叉氣泡圖。切換至具體國家即可顯示。")
...
with tab1:
    if not filtered_df.empty:
        st.dataframe(filtered_df.describe().T, use_container_width=True, height=260, column_config={"count": "📊 樣本筆數", "mean": "📈 平均值", "std": "📉 標準差", "min": "⬇️ 最小值", "25%": "¼ 25%分位", "50%": "🌓 中位數", "75%": "¾ 75%分位", "max": "⬆️ 最大值"})
        st.markdown("---")
        st.info("""
