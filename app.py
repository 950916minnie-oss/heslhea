...
def load_data():
    ...
    # 🔥 在這裡先把左邊那一排最難的英文變數名稱換成漂亮的中文對照！
    rename_dict = {
        "Average_age_of_vessels_years_Value": "⏳ 停靠船舶平均船齡 (年)",
        "Median_time_in_port_days_Value": "⏱️ 船舶在港停留時間中位數 (天)",
        "Average_size_GT_of_vessels_Value": "⚖️ 平均船舶總噸位 (GT)",
        "Average_cargo_carrying_capacity_dwt_per_vessel_Value": "📦 平均船舶載重噸位 (DWT)",
        "Average_container_carrying_capacity_TEU_per_container_ship_Value": "🧱 貨櫃船平均運載量 (TEU/20呎標準櫃)",
        "Maximum_size_GT_of_vessels_Value": "💥 停靠最大船舶總噸位 (Maximum GT)",
        "Maximum_cargo_carrying_capacity_dwt_of_vessels_Value": "💥 停靠最大船舶載重噸位 (Maximum DWT)",
        "Maximum_container_carrying_capacity_TEU_of_container_ships_Value": "💥 停靠最大貨櫃船運載量 (Maximum TEU)"
    }
...
# 注意：因為上面換了欄位中文名字，這裡對比長條圖的過濾欄位名稱也要同步更新成中文！
...
if not all_ships_data.empty:
    with col1:
        val1 = all_ships_data['⏱️ 船舶在港停留時間中位數 (天)'].values[0]
        st.metric(
            label="⏱️ 船舶在港停留時間中位數", 
    ...
    with col2:
        val2 = all_ships_data['⏳ 停靠船舶平均船齡 (年)'].values[0]
        st.metric(
            label="⏳ 停靠船舶平均船齡", 
    ...
    with col3:
        val3 = all_ships_data['⚖️ 平均船舶總噸位 (GT)'].values[0]
        st.metric(
            label="⚖️ 平均船舶總噸位 (GT)", 
...
if not vessel_comparison_df.empty:
    chart1_df = vessel_comparison_df.dropna(subset=['⏱️ 船舶在港停留時間中位數 (天)'])
    if not chart1_df.empty:
        fig_time = px.bar(
            chart1_df,
            x='CommercialMarket_Label',
            y='⏱️ 船舶在港停留時間中位數 (天)',
            labels={'CommercialMarket_Label': '船舶類型 (Vessel Type)', '⏱️ 船舶在港停留時間中位數 (天)': '在港停留天數中位數 (Days)'},
            color='⏱️ 船舶在港停留時間中位數 (天)',
...
scatter_df = vessel_comparison_df.dropna(subset=['⚖️ 平均船舶總噸位 (GT)', '📦 平均船舶載重噸位 (DWT)'])
...
if not scatter_df.empty:
    fig_scatter = px.scatter(
        ...
        labels={
            '⚖️ 平均船舶總噸位 (GT)': '平均船舶總噸位 (Average Size GT)',
            '📦 平均船舶載重噸位 (DWT)': '平均載重噸位 (Average DWT - 船隻能載多重的貨)',
...
with tab1:
    ...
    if not filtered_df.empty:
        # 這裡利用 column_config 完美將上排的 count, mean, std 一鍵換成精美中文！
        st.dataframe(
